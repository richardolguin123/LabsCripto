...
def cifrar_3des(texto_bytes, clave_bytes, iv_bytes):
    ...
    try:
        ...
        datos_con_padding = pad(texto_bytes, DES3.block_size)
        datos_cifrados = cifrador.encrypt(datos_con_padding)

        print(f"Clave final usada (bytes): {clave_ajustada}")
        print(f"IV final usado (bytes):    {iv_ajustado}")
        print(f"Texto Cifrado (bytes):   {datos_cifrados}")

        print(f"Texto Cifrado (Hex):     {datos_cifrados.hex()}")
        print(f"Texto Cifrado (Base64):  {base64.b64encode(datos_cifrados).decode('utf-8')}")

        descifrador = DES3.new(clave_ajustada, DES3.MODE_CBC, iv_ajustado)
        texto_descifrado_rellenado = descifrador.decrypt(datos_cifrados)
